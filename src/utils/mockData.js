  ...
  sessions.forEach((session) => {
    ...
    enrolledStudentIds.forEach((studentId) => {
      ...
      const record = {
        ...
        confidence:
          session.detectionMethod === "ai" ? 0.85 + Math.random() * 0.15 : 1.0,
...
export const generateMockData = () => {
  ...
  console.log(
    `Generated: ${students.length} students, ${courses.length} courses, ${sessions.length} sessions, ${attendance.length} records`
  );
